...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex">
      {/* Sidebar */}
      <aside className={cn(
        'fixed inset-y-0 left-0 z-50 w-64 bg-header shadow-2xl flex flex-col transition-transform duration-300',
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-800">
          <div className="flex items-center gap-3">
            {config?.logo_url ? (
              ...
              <span className="text-white font-bold text-lg">{config?.brand_name || 'Share'}</span>
            )}
            <span className="text-[10px] font-mono text-gray-500 bg-gray-800 px-1.5 py-0.5 rounded uppercase">Admin</span>
          </div>
